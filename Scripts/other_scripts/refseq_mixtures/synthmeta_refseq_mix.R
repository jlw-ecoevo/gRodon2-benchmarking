...
makeSim <- function(i,genome_df,n_sim){
  ...
  genes <- readDNAStringSet(paste0(as.character(genome_df$Genome[sample_i][1]),".gz"))
  ...
  for(i in 2:length(sample_i)){
    genes_i <- readDNAStringSet(paste0(as.character(genome_df$Genome[sample_i][i]),".gz"))
...
save(xm,file="refseq_synthmix_data.rda")
