...
makeSim <- function(i,genome_df,samp_size){  
  print(i)
  genome <- paste0("sim2_",i,".fna.gz")
  genomes <- as.character(xm$Genomes[paste0(as.character(xm$Genome),".gz")==genome]) %>% strsplit(split=",") %>% unlist()
  ...
  return(data.frame(d.mt.i=growthmt.i$d, 
                    ...
                    dmt_avg_i=median(c(genome_df$d.mt.i[sample_i])), 
                    dmngct_avg_i=median(c(genome_df$d.mngct.i[sample_i]))))    
...
save(xm,file="refseq_simmeta2i_data.rda")
