...
p2 <- ggplot(xmm,aes(x=reorder(variable,value),y=value,fill=variable)) +
  ...
  stat_compare_means(method="anova",label.x=1.2,label.y=1.2) +  
  stat_compare_means(label = "p.signif", method = "wilcox.test",
                     ref.group = "Genome Mixture", hide.ns = F, label.y=0.1)


setwd("~/gRodon2-benchmarking/Figures/")
png(file="Compare_zou_synth.png",width=1000,height=1000)
...
setwd("~/gRodon2-benchmarking/Figures/")
png(file="Zou_frag_CUB.png",width=500,height=500)
